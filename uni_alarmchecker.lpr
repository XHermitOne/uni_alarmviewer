{
Консольный режим проверки аварий UniAlarmChecker.
...
type
  ...
  TUniAlarmCheckerApplication = class(TCustomApplication)
  protected
...
procedure TUniAlarmCheckerApplication.DoRun;
var
  ErrorMsg: String;
begin
  ...
  engine.PRG_ENGINE := TICAlarmChecker.Create(nil);
  engine.PRG_ENGINE.Start;

  // Выполнить обработчик одного тика
  if (engine.PRG_ENGINE <> nil) then
    engine.PRG_ENGINE.Tick;
...
constructor TUniAlarmCheckerApplication.Create(TheOwner: TComponent);
begin
  inherited Create(TheOwner);
  StopOnException:=True;
end;

destructor TUniAlarmCheckerApplication.Destroy;
begin
  inherited Destroy;
end;
...
var
  Application: TUniAlarmCheckerApplication;
